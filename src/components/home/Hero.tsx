  ...
  return (
    <section className="relative h-[78vh] min-h-[560px] w-full">
      <img
        src={item.backdrop}
        alt={item.title}
        className="absolute inset-0 h-full w-full object-cover animate-pan-y"
      />
      {/* 좌→우, 아래→위 그라디언트 겹침 */}
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-28">
        ...
        <div className="mt-5 flex gap-3">
          <button
            className="inline-flex items-center gap-2 rounded bg-white px-5 py-2.5
                             text-black font-bold hover:bg-white/90"
          >
            ▶ 재생
          </button>
          <button
            className="inline-flex items-center gap-2 rounded bg-neutral-500/40 px-5 py-2.5
